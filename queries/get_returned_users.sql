...
select
    toUnixTimestamp(toDate(m.exp_start_dt)) as cohort_date,
    m.variation as variation,
    uniqExact(m.unified_id) as user_cnt,
    uniqExactIf(r.unified_id, r.datetime > toDateTime(m.exp_start_dt) and r.datetime <= toDateTime(m.exp_start_dt) + interval 24 hour and r.session_id <> m.session_id) as retention_1d_cnt,
    uniqExactIf(r.unified_id, r.datetime > toDateTime(m.exp_start_dt) and r.datetime <= toDateTime(m.exp_start_dt) + interval 168 hour and r.session_id <> m.session_id) as retention_7d_cnt,
    uniqExactIf((r.unified_id, r.session_id), r.datetime between toDateTime(m.exp_start_dt) and toDateTime({datetime_end})) as session_cnt,
    uniqExactIf(r.unified_id, r.datetime between toDateTime(m.exp_start_dt) and toDateTime({datetime_end}) and event = 'Tab View 60s') as long_tab_view_cnt
from
    default.ug_rt_events_web as r
inner join
    members_filtered as m
on
    r.unified_id = m.unified_id
where
    r.date between toDate({datetime_start}) and toDate({datetime_end}) + interval 8 day
and
    r.unified_id > 0
and
    r.session_id > 0
